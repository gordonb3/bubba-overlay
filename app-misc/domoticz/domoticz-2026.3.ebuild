...
CTIME="2026-08-02 11:10:11 +0200"
COMMIT="f734fde31"
REVCOUNT="18157"
...
src_prepare() {
	...
	sed -e "/add_subdirectory(extern\/libwebem)/d" -e "/get_target_property(_webem_type/cset(_webem_type \"dynamic\")" -i ${S}/CMakeLists.txt
	
	# check for source files that include libwebem header files directly in the extern folder
	grep -r "#include \"../extern/libwebem" | cut -d: -f1 | uniq | while read file; do
		sed -e "s/#include \".*\/extern\/libwebem\/include\/\([^\"]*\)\"/#include <\\1>/" -e "s/#include \".*\/extern\/libwebem\/src\/\([^\"]*\)\"/#include <libwebem\/\\1>/" -i $file
	done
