...
#
# Oct 2024 - gordonb3 <gordon@bosvangennip.nl>
#  - always use GPT from now on when creating a new partition table as the
#       former logic did not always result in a valid partition table with
#       new unformatted disks
#
...
create_partitions() {
	...
	STEP+=1
	echo "Step ${STEP} of ${NSTEPS}: creating new \"gpt\" partition table on /dev/${TARGETDEV}..."
...
copy_system() {
	...
	rm -rf ${TARGETROOT}/dev ${TARGETROOT}/tmp ${TARGETROOT}/bin ${TARGETROOT}/lib ${TARGETROOT}/sbin
	cp -ax ${SOURCEROOT}/dev ${SOURCEROOT}/tmp ${SOURCEROOT}/bin ${SOURCEROOT}/lib ${SOURCEROOT}/sbin ${TARGETROOT} >>"${LOG}" 2>&1
	for dir in etc opt root usr; do
